...
Set the background color or image of the bar. I<color> might be either in hex format (#aarrggbb) or in the symbolic name format (eg. white, brightred, darkgray). If the argument ends in I<.png> it will be loaded as an image. If no compositor such as compton or xcompmgr is running the alpha channel is silently ignored.
...
Load an image. Currently only PNG files are supported.
